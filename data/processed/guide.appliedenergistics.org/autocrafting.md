...
2. The ME Pattern Provider with the relevant [pattern](/1.21.1/items-blocks-machines/patterns) pushes the ingredients specified in the pattern to any adjacent inventory.
...
3. The result of the craft is returned to the system somehow, be it by import bus, interface, or pushing the result back into a pattern provider.
...
in vanilla Minecraft. However, there is [a way to handle these recipes.](/1.21.1/example-setups/recursive-crafting-setup)
...
Multiple ME Pattern Providers with identical patterns are supported and work in parallel. Additionally, you can have a pattern say,
...
There is actually an even more "general" form of "pattern" than a processing pattern. A ME Level Emitter with a crafting card can be set
...
instead of using an ME Import Bus to pull the output of the machine into the network.

Of note, since they push the ingredients directly from the [crafting storage](/1.21.1/items-blocks-machines/crafting_cpu_multiblock#crafting-storage) in a crafting CPU, they
...
Pattern providers have a special interaction with interfaces on [subnets](/1.21.1/ae2-mechanics/subnetworks): if the interface is unmodified (nothing in the request slots)
...
Priorities can be set by clicking the wrench in the top-right of the GUI. In the case of several [patterns](/1.21.1/items-blocks-machines/patterns)
...
The Molecular Assembler takes items input into it and carries out the operation defined by an adjacent ME Pattern Provider,
...
Their main use is next to a ME Pattern Provider. Pattern providers have special behavior in this case,
